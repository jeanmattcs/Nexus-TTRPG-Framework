...
namespace EpicTransport {
    ...
    public class EOSSDKComponent : MonoBehaviour {
        ...
        private void Awake() {
            ...
            {
                "Assets/Thirdparty/Mirror/Runtime/Transport/EpicOnlineTransport/EOSSDK/",
                "Assets/Mirror/Runtime/Transport/EpicOnlineTransport/EOSSDK/"
            };
            // Try Config.LibraryName first, then platform-specific fallbacks
            System.Collections.Generic.List<string> candidates = new System.Collections.Generic.List<string>();
            candidates.Add(Config.LibraryName);

            // Add platform-specific names to guard against incorrect compile-time detection
            if (Application.platform == RuntimePlatform.LinuxEditor || Application.platform == RuntimePlatform.LinuxPlayer)
            {
                candidates.Add("libEOSSDK-Linux-Shipping.so");
                candidates.Add("libEOSSDK-Linux-Shipping");
            }
            else if (Application.platform == RuntimePlatform.OSXEditor || Application.platform == RuntimePlatform.OSXPlayer)
            {
                candidates.Add("libEOSSDK-Mac-Shipping.dylib");
                candidates.Add("libEOSSDK-Mac-Shipping");
            }
            else
            {
                candidates.Add(IntPtr.Size == 8 ? "EOSSDK-Win64-Shipping" : "EOSSDK-Win32-Shipping");
            }
            ...
            if (libraryPointer == IntPtr.Zero)
            {
                throw new Exception("Failed to load EOSSDK library. Tried: " + string.Join(", ", new string[] { basePaths[0] + (candidates.Count > 0 ? candidates[0] : string.Empty), basePaths[1] + (candidates.Count > 0 ? candidates[0] : string.Empty) }));
            }
#endif
        ...
        private void ConnectInterfaceLogin() {
            ...
            // For DeviceId flow the token may be null; some SDK versions reject null pointers.
            // Provide empty string to satisfy parameter validation while using DeviceId.
            if (connectInterfaceCredentialType == Epic.OnlineServices.ExternalCredentialType.DeviceidAccessToken && string.IsNullOrEmpty(connectInterfaceCredentialToken))
            {
                loginOptions.Credentials.Token = string.Empty;
            }
            else
            {
